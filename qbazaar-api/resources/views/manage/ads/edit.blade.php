...
@section('content')
    <a href="{{ route('manage.ads.show', $ad) }}" class="mb-4 inline-flex items-center gap-1.5 text-sm font-semibold text-ink-500 transition hover:text-coral">
        <x-manage.icon name="arrow-right" class="size-4" /> رجوع للإعلان
    </a>

    <form method="POST" action="{{ route('manage.ads.update', $ad) }}" class="max-w-3xl space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="flex items-center gap-3">
            <button type="submit" class="inline-flex items-center gap-2 rounded-xl bg-coral px-5 py-2.5 text-sm font-bold text-white transition hover:brightness-95">
                <x-manage.icon name="check" class="size-[18px]" /> حفظ التعديلات
            </button>
            <a href="{{ route('manage.ads.show', $ad) }}" class="text-sm font-semibold text-ink-500 hover:text-coral">إلغاء</a>
        </div>
    </form>
@endsection
