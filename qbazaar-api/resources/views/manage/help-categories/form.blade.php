...
@section('content')
    ...
    <form method="POST"
          action="{{ $isEdit ? route('manage.help-categories.update', $category) : route('manage.help-categories.store') }}"
        ...
        <div class="flex items-center gap-3">
            <button type="submit" class="rounded-xl bg-coral px-5 py-2.5 text-sm font-bold text-white hover:brightness-95">حفظ</button>
            <a href="{{ route('manage.help-categories.index') }}" class="text-sm font-semibold text-ink-500 hover:text-coral">إلغاء</a>
        </div>
    </form>
@endsection
