...
@section('content')
    ...
    <div class="grid grid-cols-2 gap-4 md:grid-cols-4">
        @foreach ([
            ['نشطة', $ads['active'], 'check', 'admin.ads.index', ['status' => 'active']],
            ['بانتظار المراجعة', $ads['pending'], 'clock', 'admin.ads.index', ['status' => 'pending']],
            ['نُشرت اليوم', $ads['published_today'], 'trend', null, null],
            ['الإجمالي', $ads['total'], 'tag', 'admin.ads.index', []],
    ...
    <div class="mt-8 grid gap-6 lg:grid-cols-2">
        ...
        <div class="rounded-2xl border border-ink-200 bg-cream-100 p-6">
            <h2 class="mb-4 flex items-center gap-2 font-bold"><x-admin.icon name="trend" class="size-5 text-coral" /> الإعلانات المنشورة — آخر ١٤ يوماً</h2>
        ...
        <div class="rounded-2xl border border-ink-200 bg-cream-100">
            <div class="flex items-center justify-between border-b border-ink-200 px-6 py-4">
                ...
                <a href="{{ route('admin.ads.index', ['status' => 'pending']) }}" class="text-sm font-semibold text-coral">عرض الكل</a>
            </div>
                ...
                <ul class="divide-y divide-ink-200">
                    @foreach ($pendingAds as $ad)
                        <li class="flex items-center justify-between px-6 py-3.5">
                            <div class="min-w-0">
                                <a href="{{ route('admin.ads.show', $ad) }}" class="block truncate font-semibold hover:text-coral">{{ $ad->title }}</a>
                                <div class="mt-0.5 text-xs text-ink-500">{{ $ad->user?->full_name ?? '—' }} · {{ optional($ad->created_at)->diffForHumans() }}</div>
                            </div>
                            <a href="{{ route('admin.ads.show', $ad) }}" class="shrink-0 rounded-lg bg-cream-200 px-3 py-1.5 text-xs font-semibold hover:bg-ink-200">مراجعة</a>
                        </li>
                    @endforeach
                </ul>
            @endif
