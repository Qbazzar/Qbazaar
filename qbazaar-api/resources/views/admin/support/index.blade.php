...
@section('content')
    ...
    <form method="GET" class="mb-6 flex flex-wrap items-center gap-3">
        ...
        @if ($search !== '' || $status !== '' || $priority !== '')
            <a href="{{ route('admin.support.index') }}" class="text-sm font-semibold text-ink-500 hover:text-coral">مسح</a>
        @endif
    </form>
                ...
                <tbody class="divide-y divide-ink-200">
                    @forelse ($tickets as $ticket)
                        <tr class="hover:bg-cream-50">
                            <td class="px-4 py-3">
                                <a href="{{ route('admin.support.show', $ticket) }}" class="font-semibold hover:text-coral">{{ \Illuminate\Support\Str::limit($ticket->subject, 60) }}</a>
                            </td>
                            ...
                            <td class="px-4 py-3 text-left">
                                <div class="flex items-center justify-end gap-1.5">
                                    <a href="{{ route('admin.support.show', $ticket) }}" title="عرض"
                        ...
                        <tr>
                            <td colspan="7" class="px-4 py-16 text-center">
                                <span class="mx-auto mb-3 flex size-12 items-center justify-center rounded-full bg-cream-200 text-ink-300">
                                    <x-admin.icon name="lifebuoy" class="size-6" />
                                </span>
