...
@section('content')
    ...
    <form method="GET" class="mb-6 flex flex-wrap items-center gap-3">
        ...
        @if ($search !== '' || $status !== '' || $role !== '')
            <a href="{{ route('admin.users.index') }}" class="text-sm font-semibold text-ink-500 hover:text-coral">مسح</a>
        @endif
    </form>
                ...
                <tbody class="divide-y divide-ink-200">
                    @forelse ($users as $user)
                        <tr class="transition hover:bg-cream-50">
                            <td class="px-4 py-3">
                                <a href="{{ route('admin.users.show', $user) }}" class="font-semibold hover:text-coral">{{ $user->full_name ?? '—' }}</a>
                            </td>
                            ...
                            <td class="px-4 py-3 text-left">
                                <div class="flex items-center justify-end gap-1.5">
                                    <a href="{{ route('admin.users.show', $user) }}" title="عرض"
                        ...
                        <tr>
                            <td colspan="7" class="px-4 py-16 text-center">
                                <span class="mx-auto mb-3 flex size-12 items-center justify-center rounded-full bg-cream-200 text-ink-300">
                                    <x-admin.icon name="users" class="size-6" />
                                </span>
