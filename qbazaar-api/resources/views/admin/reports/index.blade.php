...
@section('content')
    ...
    <form method="GET" class="mb-6 flex flex-wrap items-center gap-3">
        ...
        @if ($search !== '' || $status !== '' || $category !== '')
            <a href="{{ route('admin.reports.index') }}" class="text-sm font-semibold text-ink-500 hover:text-coral">مسح</a>
        @endif
    </form>

    @include('admin.partials.bulk-bar', ['action' => route('admin.reports.bulk-dismiss'), 'method' => 'POST', 'icon' => 'x-circle', 'tone' => 'coral', 'label' => 'رفض المحدد', 'confirm' => 'رفض البلاغات المحددة (المعلّقة فقط)؟'])
                        ...
                        <tr class="transition hover:bg-cream-50">
                            ...
                            <td class="px-4 py-3 text-left">
                                <div class="flex items-center justify-end gap-1.5">
                                    <a href="{{ route('admin.reports.show', $report) }}" title="عرض"
                        ...
                        <tr>
                            <td colspan="7" class="px-4 py-16 text-center">
                                <span class="mx-auto mb-3 flex size-12 items-center justify-center rounded-full bg-cream-200 text-ink-300">
                                    <x-admin.icon name="flag" class="size-6" />
                                </span>
